...
class TestIngestCodeGneration(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        prop_a_1 = Property(name="uniqueProp1", csv_mapping="unique_prop_1", type="str", is_unique=True)
        prop_a_2 = Property(name="prop1", csv_mapping="prop_1", type="str", is_unique=False)
        prop_b_1 = Property(name="uniqueProp2", csv_mapping="unique_prop_2", type="str", is_unique=True)
        prop_b_2 = Property(name="prop2", csv_mapping="prop_2", type="str", is_unique=False)
        prop_rel_1 = Property(name="relProp", csv_mapping="rel_prop", type="int", is_unique=False)

        cls.node_a = Node(label="NodeA", properties=[prop_a_1, prop_a_2])
        cls.node_b = Node(label="NodeB", properties=[prop_b_1, prop_b_2])
        cls.rel_1 = Relationship(type="HAS_RELATIONSHIP", properties=[prop_rel_1], source=cls.node_a, target=cls.node_b)

        cls.data_model = DataModel(nodes=[cls.node_a, cls.node_b], relationships=[cls.rel_1])
