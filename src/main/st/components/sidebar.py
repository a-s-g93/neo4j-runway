...
def sidebar(content_file_path: str) -> None:
    ...
    if st.session_state["summarizer"] is not None:
        with st.sidebar.form("Data Model Version Select"):
            csv_dir = st.text_input(label="Local CSV Location", value="./", placeholder="where is this file found?")
            ...
            if st.form_submit_button("Generate Ingestion Code"):
                ...
                st.session_state["ingestion_generator"] = IngestionGenerator(data_model=st.session_state["summarizer"].model_history[version].dict,
