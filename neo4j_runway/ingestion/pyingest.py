...
def PyIngest(
    yaml_string: Union[str, None] = None,
    ...
    **kwargs
    ...
    """
    if "config" in kwargs:
        load_config(get_yaml(kwargs["config"]))
    else:
        load_config(get_yaml(yaml_string))
        warnings.warn(
            "the yaml_string parameter will be depreciated in future releases. Please use the 'config' kwarg to identify the YAML file instead."
        )
    server = LocalServer()
    server.pre_ingest()
