...
class TestRelationship(unittest.TestCase):
    ...
    def test_init(self) -> None:
        ...
        self.assertEquals(relationship.type, "HAS_SIMILAR")
        self.assertEquals(len(relationship.properties), 2)

    def test_properties(self) -> None:

        relationship = Relationship(type="HAS_SIMILAR", properties=[self.prop1, self.prop2], source=self.source, target=self.target)

        self.assertEquals(relationship.property_names, ["score", "current"])

    def test_unique_constraints(self) -> None:

        relationship = Relationship(type="HAS_SIMILAR", properties=[self.prop1, self.prop2], source=self.source, target=self.target)

        self.assertEquals(relationship.unique_constraints, ["current"])

    def test_property_column_mapping(self) -> None:

        relationship = Relationship(type="HAS_SIMILAR", properties=[self.prop1, self.prop2], source=self.source, target=self.target)

        self.assertEquals(relationship.property_column_mapping, {"score": "similarity_score", "current": "current"})

    def test_unique_constraints_column_mapping(self) -> None:

        relationship = Relationship(type="HAS_SIMILAR", properties=[self.prop1, self.prop2], source=self.source, target=self.target)

        self.assertEquals(relationship.unique_constraints_column_mapping, {"current": "current"})
